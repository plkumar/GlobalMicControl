...
{
	std::wstring strToolTip = std::wstring(lpszToolTip);

	ASSERT((strToolTip.length() > 0 && strToolTip.length() < 64));
	if ((strToolTip.length() > 0 && strToolTip.length() < 64))
	{
		StrCpyW(m_nidIconData.szTip, lpszToolTip);
		//strcpy_w(m_nidIconData.szTip,lpszToolTip);
...
BOOL CTrayDialog::TrayShow()
{
	...
	{
		if (m_bAlwaysShowTrayIcon) bSuccess = TRUE;
...
void CTrayDialog::TraySetAlwaysVisible(BOOL bHideTray)
{
	m_bAlwaysShowTrayIcon = bHideTray;
}
...
void CTrayDialog::OnTrayLButtonDblClk(CPoint pt)
{
	if (m_bMinimizeToTray)
	{
		if (m_bAlwaysShowTrayIcon == FALSE) 
			TrayHide();
